...
import { hexPoints, getVertexPixel, getEdgePoints, HEX_SIZE, CATAN_HEX_COORDS, hexId } from '../../lib/catan/svgHelpers.js';
...
describe('hexPoints', () => {
  ...
  it('works with non-zero center', () => {
    ...
    const pairs0 = pts0.trim().split(' ').map(p => p.split(',').map(Number));
    const pairs1 = pts1.trim().split(' ').map(p => p.split(',').map(Number));
    for (let i = 0; i < 6; i++) {
