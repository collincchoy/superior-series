  ...
  describe("ACTION phase with pending actions", () => {
    ...
    it("build_settlement returns valid empty vertices connected to road network", () => {
      ...
      state = applyAction(state, {
        type: "PLACE_ROAD",
        pid: "p2",
        eid: (graph.edgesOfVertex[vids[10]!] ?? [])[0]!,
      });
      state = applyAction(state, {
        type: "PLACE_BUILDING",
        pid: "p1",
      ...
      const e1FarVid = (graph.verticesOfEdge[e1] ?? []).find(
        (v) => v !== settleVid,
      ) as VertexId;
      const extraEdge = (graph.edgesOfVertex[e1FarVid] ?? []).find(
        (e) => e !== e1,
      ) as EdgeId;
    ...
    it("build_city_wall returns only own unwalled city vertices", () => {
      ...
      state = applyAction(state, {
        type: "PLACE_ROAD",
        pid: "p1",
        eid: (graph.edgesOfVertex[vids[0]!] ?? [])[0]!,
      });
      state = applyAction(state, {
        type: "PLACE_BUILDING",
        pid: "p2",
  ...
  describe("Progress card and knight move selections", () => {
    ...
    beforeEach(() => {
      ...
      state = applyAction(state, {
        type: "PLACE_ROAD",
        pid: "p2",
        eid: (graph.edgesOfVertex[vids[20]!] ?? [])[0]!,
      });
      state = applyAction(state, {
        type: "PLACE_BUILDING",
        pid: "p1",
      ...
      state = applyAction(state, {
        type: "PLACE_ROAD",
        pid: "p1",
        eid: (graph.edgesOfVertex[vids[25]!] ?? [])[0]!,
      });
    ...
    it("progress_select_hex for Taxation returns empty when robber is inactive", () => {
      ...
      const targets = computeValidTargets(inactiveRobberState, "p1", {
        type: "progress_select_hex",
        card: "Taxation",
      });
      expect(targets.validHexes.size).toBe(0);
