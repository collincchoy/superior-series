...
import type { BoardState, HexId, VertexId, EdgeId, GameState, Player } from '../../lib/catan/types.js';
...
function makePlayer(id: string, overrides: Partial<Player> = {}): Player {
  return {
    ...
    color: '#ff0000',
    isBot: false,
    resources: { ...emptyResources(), brick: 5, lumber: 5, ore: 5, grain: 5, wool: 5, cloth: 5, coin: 5, paper: 5 },
    progressCards: [],
    vpTokens: 0,
    improvements: { science: 0, trade: 0, politics: 0 },
    supply: {
      roads: 15, settlements: 4, cities: 4, cityWalls: 3,
      knights: { 1: 2, 2: 2, 3: 2 },
    },
...
function getConnectedVertexAndEdge(startVertex: VertexId): { vertex: VertexId; edge: EdgeId } {
...
describe('canBuildSettlement (action phase)', () => {
  it('fails if no settlement supply', () => {
    const board = makeBoard();
    const player = makePlayer('p1', { supply: { roads: 15, settlements: 0, cities: 4, cityWalls: 3, knights: { 1: 2, 2: 2, 3: 2 } } });
    const vid = getFirstVertex();
    const edges = graph.edgesOfVertex[vid]!;
    board.edges[edges[0]!] = { playerId: 'p1' };
...
describe('canBuildCityWall', () => {
  it('succeeds on own city without wall', () => {
    ...
    const player = makePlayer('p1');
    const vid = getFirstVertex();
    board.vertices[vid] = { type: 'city', playerId: 'p1', hasWall: false, metropolis: null };
  ...
  it('fails on city that already has a wall', () => {
    ...
    const player = makePlayer('p1');
    const vid = getFirstVertex();
    board.vertices[vid] = { type: 'city', playerId: 'p1', hasWall: true, metropolis: null };
    expect(canBuildCityWall(board, player, vid)).toBe(false);
  });

  it('fails when no city walls in supply', () => {
    const board = makeBoard();
    const player = makePlayer('p1', { supply: { roads: 15, settlements: 4, cities: 4, cityWalls: 0, knights: { 1: 2, 2: 2, 3: 2 } } });
    const vid = getFirstVertex();
    board.vertices[vid] = { type: 'city', playerId: 'p1', hasWall: false, metropolis: null };
    expect(canBuildCityWall(board, player, vid)).toBe(false);
  });
...
describe('canRecruitKnight', () => {
  ...
  it('fails with no basic knights in supply', () => {
    const board = makeBoard();
    const player = makePlayer('p1', { supply: { roads: 15, settlements: 4, cities: 4, cityWalls: 3, knights: { 1: 0, 2: 2, 3: 2 } } });
    const vid = getFirstVertex();
    board.vertices[vid] = { type: 'settlement', playerId: 'p1' };
    const edges = graph.edgesOfVertex[vid]!;
    board.edges[edges[0]!] = { playerId: 'p1' };
...
describe('canDisplaceKnight', () => {
  it('succeeds when player knight is stronger than target', () => {
    ...
    expect(canDisplaceKnight(board, graph, 'p1', fromVid, targetVid)).toBe(true);
  });
  ...
  it('fails when player knight is equal strength', () => {
    ...
    expect(canDisplaceKnight(board, graph, 'p1', fromVid, targetVid)).toBe(false);
  });
