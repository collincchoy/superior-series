...
import { createInitialState, applyAction, computeVP } from '../../lib/catan/game.js';
...
describe('createInitialState', () => {
  ...
  it('robber is not on any hex at start (robberActive = false)', () => {
    const state = createInitialState(makePlayers(3));
    const hexWithRobber = Object.values(state.board.hexes).find(h => h.hasRobber);
...
describe('setup phase - PLACE_BUILDING', () => {
  it('SETUP_R1: places a settlement on the board', () => {
    ...
    const nextState = applyAction(state, {
      type: 'PLACE_BUILDING', pid: 'p1', vid, building: 'settlement',
    });
  ...
  it('SETUP_R2: places a city (not settlement)', () => {
    ...
    const nextState = applyAction(state, {
      type: 'PLACE_BUILDING', pid: state.currentPlayerId, vid, building: 'city',
    });
  ...
  it('SETUP_R2: grants starting resources for city placement', () => {
    ...
    const nextState = applyAction(state, {
      type: 'PLACE_BUILDING', pid, vid, building: 'city',
    });
    ...
    const totalResources = Object.values(nextState.players[pid]!.resources).reduce((a, b) => a + b, 0);
...
describe('resource production', () => {
  it('settlement on hex with rolled number gets 1 resource', () => {
    ...
    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid, result: [hexNum - 1, 1, 'politics' as const],
    });
    // After production, player should have more resources
    const after = rolled.players[pid]!.resources;
    const gained = Object.entries(after).reduce((sum, [k, v]) => sum + v - (before[k as keyof typeof before] ?? 0), 0);
  ...
  it('rolling 7 triggers discard for players with 8+ cards', () => {
    ...
    state = {
      ...
      players: {
        ...state.players,
        p2: { ...state.players['p2']!, resources: { ...state.players['p2']!.resources, brick: 10 } },
      },
    };
    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid, result: [3, 4, 'ship' as const], // sum = 7
    });
  ...
  it('rolling 7 does NOT move robber when robber is inactive', () => {
    ...
    expect(state.barbarian.robberActive).toBe(false);
    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid, result: [3, 4, 'ship' as const],
    });
    // Robber should still be inactive and no hex should have it
    expect(rolled.barbarian.robberActive).toBe(false);
    const hexWithRobber = Object.values(rolled.board.hexes).find(h => h.hasRobber);
    expect(hexWithRobber).toBeUndefined();
  });
});
...
describe('computeVP', () => {
  ...
  it('metropolis on city adds +2 VP (total 4 for that city)', () => {
    ...
    const cityVertex = Object.entries(state.board.vertices)
      .find(([, b]) => b?.type === 'city' && b.playerId === pid)?.[0] as VertexId | undefined;
    ...
    const stateWithMetro = {
      ...
      board: {
        ...
        vertices: {
          ...
          [cityVertex]: { type: 'city' as const, playerId: pid, hasWall: false, metropolis: 'science' as const },
        },
      },
    };
...
function buildSetupR2State(): GameState {
  ...
  while (state.phase === 'SETUP_R1_SETTLEMENT' || state.phase === 'SETUP_R1_ROAD') {
    if (state.phase === 'SETUP_R1_SETTLEMENT') {
      ...
      if (!vid) break;
      state = applyAction(state, { type: 'PLACE_BUILDING', pid: state.currentPlayerId, vid, building: 'settlement' });
    } else {
      // Place road adjacent to the last placed building
      const lastBuilding = Object.entries(state.board.vertices)
        .find(([, b]) => b?.playerId === state.currentPlayerId && b.type === 'settlement')?.[0] as VertexId | undefined;
      if (!lastBuilding) break;
      const freeEdge = (graph.edgesOfVertex[lastBuilding] ?? []).find(e => state.board.edges[e] === null);
      if (!freeEdge) break;
      state = applyAction(state, { type: 'PLACE_ROAD', pid: state.currentPlayerId, eid: freeEdge });
    }
  }
  return state;
}
...
function buildActionState(): GameState {
  ...
  // Complete full setup
  while (['SETUP_R1_SETTLEMENT', 'SETUP_R1_ROAD', 'SETUP_R2_CITY', 'SETUP_R2_ROAD'].includes(state.phase)) {
    const pid = state.currentPlayerId;
    if (state.phase === 'SETUP_R1_SETTLEMENT') {
      let vid: VertexId | null = null;
      for (let i = vIdx; i < allVertices.length; i++) {
        ...
        if (state.board.vertices[v] === null) {
          const adj = graph.adjacentVertices[v] ?? [];
          if (adj.every(a => state.board.vertices[a] === null)) {
            vid = v; vIdx = i + 1; break;
          }
        }
      }
      if (!vid) break;
      state = applyAction(state, { type: 'PLACE_BUILDING', pid, vid, building: 'settlement' });
      ...
      if (!vid) break;
      state = applyAction(state, { type: 'PLACE_BUILDING', pid, vid, building: 'city' });
  ...
  for (const pid of state.playerOrder) {
    state = {
      ...
      players: {
        ...
        [pid]: {
          ...state.players[pid]!,
          resources: { brick: 5, lumber: 5, ore: 5, grain: 5, wool: 5, cloth: 5, coin: 5, paper: 5 },
        },
      },
    };
  }
...
function buildActionStateWithCity(): GameState | null {
  ...
  const hasCities = Object.values(state.board.vertices).some(b => b?.type === 'city');
  if (hasCities) return state;
  return null;
}

function findLastPlacedBuilding(state: GameState, pid: PlayerId, type: 'settlement' | 'city'): VertexId | null {
  // Return any building vertex for this player of the given type
  const entry = Object.entries(state.board.vertices).find(([, b]) => b?.playerId === pid && b.type === type);
...
function findCityVertexWithProduction(state: GameState): VertexId | null {
  ...
  for (const v of allVertices) {
    if (state.board.vertices[v] === null) {
      ...
      if (hexes.some(hid => state.board.hexes[hid]?.terrain !== 'desert')) return v;
    }
  }
  return null;
}

function findSettlementAndHex(state: GameState, pid: PlayerId): { hexNum: number; vid: VertexId } {
...
function findCityAndHex(state: GameState, pid: PlayerId): { hexNum: number; vid: VertexId } {
  for (const [vid, building] of Object.entries(state.board.vertices)) {
