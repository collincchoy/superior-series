...
function stateWithBarbarian(barbarianPos: number, robberActive = false): GameState {
      ...
      state = {
        ...
        board: {
          ...
          vertices: {
            ...
            [vid]: { type: 'city' as const, playerId: pid, hasWall: false, metropolis: null },
          },
        },
      };
    }
...
function addActiveKnights(state: GameState, pid: PlayerId, strength: 1 | 2 | 3, count: number): GameState {
  const allVertices = Object.keys(graph.vertices) as VertexId[];
  let vIdx = 0;
  for (let i = 0; i < count; i++) {
    let vid: VertexId | null = null;
...
describe('barbarian track', () => {
  it('advances on event die ship face', () => {
    ...
    const rolled = applyAction({ ...state, phase: 'ROLL_DICE' }, {
      type: 'ROLL_DICE', pid: 'p1', result: [2, 3, 'ship'],
    });
  ...
  it('does NOT advance on color event die face', () => {
    ...
    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid: 'p1', result: [2, 3, 'science'],
    });
...
describe('barbarian attack - defenders win', () => {
  it('defenders win: active knight strength >= city count', () => {
    ...
    state = { ...state, phase: 'ROLL_DICE' as const, barbarian: { position: 6, robberActive: false } };

    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid: 'p1', result: [2, 3, 'ship'],
    });
    // Cities should NOT be pillaged
    const cityCount = Object.values(rolled.board.vertices).filter(b => b?.type === 'city').length;
...
describe('barbarian attack - barbarians win', () => {
  it('player with 0 knights loses a city', () => {
    ...
    const before = Object.values(state.board.vertices).filter(b => b?.type === 'city').length;
    const rolled = applyAction(state, {
      type: 'ROLL_DICE', pid: 'p1', result: [2, 3, 'ship'],
    });
    const after = Object.values(rolled.board.vertices).filter(b => b?.type === 'city').length;
  ...
  it('pillaged city becomes a settlement', () => {
    ...
    const cityVertex = Object.entries(state.board.vertices)
      .find(([, b]) => b?.type === 'city')?.[0] as VertexId | undefined;
  ...
  it('metropolis city is NOT pillaged', () => {
    ...
    state = {
      ...
      board: {
        ...
        vertices: {
          ...state.board.vertices,
          [cityVertex]: { type: 'city', playerId: 'p1', hasWall: false, metropolis: 'science' },
        },
      },
  ...
  it('city wall is removed when city is pillaged', () => {
    ...
    state = {
      ...
      board: {
        ...
        vertices: {
          ...state.board.vertices,
          [vid]: { type: 'city', playerId: 'p1', hasWall: true, metropolis: null },
        },
      },
      phase: 'ROLL_DICE' as const,
      barbarian: { position: 6, robberActive: false },
  ...
  it('first barbarian attack activates the robber on the desert', () => {
    ...
    const desertHex = Object.values(rolled.board.hexes).find(h => h.terrain === 'desert');
    if (desertHex) {
