...
import type { GameState, TurnPhase, PlayerId, VertexId } from '../../lib/catan/types.js';
...
function buildActionState(): GameState {
  ...
  const setupPhases: TurnPhase[] = ['SETUP_R1_SETTLEMENT', 'SETUP_R1_ROAD', 'SETUP_R2_CITY', 'SETUP_R2_ROAD'];
...
describe('chooseBotAction - basic validity', () => {
  ...
  it('returns END_TURN or a build action during ACTION phase', () => {
    ...
    expect(action).toBeDefined();
    const validTypes = ['BUILD_ROAD', 'BUILD_SETTLEMENT', 'BUILD_CITY', 'BUILD_CITY_WALL',
      'RECRUIT_KNIGHT', 'PROMOTE_KNIGHT', 'ACTIVATE_KNIGHT', 'IMPROVE_CITY',
      'TRADE_BANK', 'END_TURN'];
  ...
  it('discards to at most 7 cards when in DISCARD phase', () => {
    ...
    state = {
      ...
      players: {
        ...
        [pid]: {
          ...state.players[pid]!,
          resources: { brick: 10, lumber: 0, ore: 0, grain: 0, wool: 0, cloth: 0, coin: 0, paper: 0 },
        },
      },
    ...
    if (action.type === 'DISCARD') {
      const total = Object.values(action.cards).reduce((a, b) => a + (b ?? 0), 0);
      expect(total).toBe(5);
    }
  });
});
...
describe('chooseBotAction - robber', () => {
  it('returns MOVE_ROBBER during ROBBER_MOVE phase', () => {
    const state = { ...buildActionState(), phase: 'ROBBER_MOVE' as const, barbarian: { position: 0, robberActive: true } };
    const action = chooseBotAction(state, state.currentPlayerId);
