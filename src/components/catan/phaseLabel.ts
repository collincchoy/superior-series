...
export function phaseLabel(state: GameState, localPid: PlayerId): string {
  ...
  const labels: Partial<Record<TurnPhase, string>> = {
    ...
    RESOLVE_PROGRESS_DRAW: isMe ? 'Draw a progress card' : 'Waiting for progress card draws…',
    KNIGHT_DISPLACE_RESPONSE: isMe ? 'Move your displaced knight' : 'Waiting for displaced knight move…',
