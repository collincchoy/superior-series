<script lang="ts">
  import type { GameState, PlayerId, VertexId, EdgeId, HexId } from '../../lib/catan/types.js';
  ...
  function onEdgeClick(eid: EdgeId) {
    ...
    if (gameState.phase === 'SETUP_R1_ROAD' || gameState.phase === 'SETUP_R2_ROAD') {
  ...
  function onHexClick(hid: HexId) {
    ...
    const pid = localPid;
    const adjacentBuildings = Object.entries(gameState.board.vertices)
      .filter(([vid, b]) => {
  ...
  let harborPositions = $derived(gameState.board.harbors.map(harbor => {
    ...
    const perpX1 = -dy / len * 30, perpY1 = dx / len * 30;
    const perpX2 = dy / len * 30,  perpY2 = -dx / len * 30;
  ...
  }).filter(Boolean) as Array<{harbor: typeof gameState.board.harbors[0]; p1:{x:number;y:number}; p2:{x:number;y:number}; outerX:number; outerY:number}>);
</script>

<div class="board-area">
  <svg class="board-svg" viewBox="-420 -400 840 800" xmlns="http://www.w3.org/2000/svg">
    <defs>
    ...
    <g id="numbers">
      {#each CATAN_HEX_COORDS as coord}
        ...
        {#if hex && hex.number !== null}
          ...
          {#if hex.hasRobber}
            <text x={x} y={y + 5} text-anchor="middle" font-size="28">👺</text>
          {:else}
            <circle cx={x} cy={y} r="18" fill="#f5e6c8" stroke="#8b6914" stroke-width="1.5" />
            <text x={x} y={y + 1} text-anchor="middle" dominant-baseline="middle"
              font-size="15" font-weight="bold" fill={isRed ? '#cc2200' : '#2c1a0a'}>
              {hex.number}
            </text>
    ...
    <g id="harbors">
      {#each harborPositions as hp}
        <line x1={hp.p1.x} y1={hp.p1.y} x2={hp.outerX} y2={hp.outerY}
          stroke="#5dade2" stroke-width="1.5" stroke-dasharray="4 3" opacity="0.7" />
        <line x1={hp.p2.x} y1={hp.p2.y} x2={hp.outerX} y2={hp.outerY}
          stroke="#5dade2" stroke-width="1.5" stroke-dasharray="4 3" opacity="0.7" />
        <circle cx={hp.outerX} cy={hp.outerY} r="13" fill="#1a5276" stroke="#5dade2" stroke-width="1.5" />
        <text x={hp.outerX} y={hp.outerY + 1} text-anchor="middle" dominant-baseline="middle"
    ...
    <g id="buildings">
      ...
      {#each Object.entries(gameState.board.vertices) as [vid, building]}
        {#if building}
          ...
          {#if p}
            {@const color = gameState.players[building.playerId]?.color ?? '#999'}
            {#if building.type === 'settlement'}
              <!-- Settlement: square base + triangle roof -->
              <rect x={p.x - 8} y={p.y - 4} width="16" height="12" fill={color} stroke="#fff" stroke-width="1.5" />
              <polygon points="{p.x},{p.y - 16} {p.x + 10},{p.y - 4} {p.x - 10},{p.y - 4}"
                fill={color} stroke="#fff" stroke-width="1.5" />
            {:else if building.type === 'city'}
              <!-- City: wide base + tower -->
              <rect x={p.x - 12} y={p.y - 8} width="24" height="16" fill={color} stroke="#fff" stroke-width="2" />
              <rect x={p.x - 6} y={p.y - 18} width="12" height="10" fill={color} stroke="#fff" stroke-width="2" />
              {#if building.hasWall}
                <rect x={p.x - 16} y={p.y + 8} width="32" height="5" fill="#8b6914" stroke="#fff" stroke-width="1" />
              {/if}
              {#if building.metropolis !== null}
                <text x={p.x} y={p.y - 20} text-anchor="middle" font-size="12">👑</text>
              {/if}
            {/if}
          {/if}
        {/if}
      ...
      {#each Object.entries(gameState.board.knights) as [vid, knight]}
        {#if knight}
          ...
          {#if p}
            ...
            <g transform={knight.active ? undefined : `rotate(90,${p.x},${p.y})`}>
              <circle cx={p.x} cy={p.y} r="12" fill={color} stroke="#fff" stroke-width="2" />
              <text x={p.x} y={p.y + 2} text-anchor="middle" dominant-baseline="middle" font-size="12">
    ...
    <g id="tokens">
      {#if gameState.board.merchantHex}
        {@const coord = CATAN_HEX_COORDS.find(c => hexId(c) === gameState.board.merchantHex)}
        {#if coord}
          {@const { x, y } = hexCenter(coord)}
